...
export class AgenticRAGService {
     ...
     */
    public static async *runStream(question: string, tenantId: string, correlationId: string, messages: any[] = []) {
     ...
     */
    public static async run(question: string, tenantId: string, correlationId: string, messages: any[] = []) {
        const workflow = this.createWorkflow();
        const app = workflow.compile();
