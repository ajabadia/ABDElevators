...
export class BillingService {
    ...
    /**
     * Simula el cambio de plan y calcula prorrateos.
     */
    static async simulatePlanChange(tenantId: string, newPlanSlug: string) {
        const tier = newPlanSlug.toUpperCase();
        if (!(tier in PRICING_PLANS)) {
            throw new ValidationError(`Plan inválido: ${newPlanSlug}`);
        }

        const newPlan = PRICING_PLANS[tier as keyof typeof PRICING_PLANS];
        const currentConfig = await TenantService.getConfig(tenantId);
        const currentTier = (currentConfig.subscription?.tier as keyof typeof PRICING_PLANS) || 'FREE';
        const currentPlan = PRICING_PLANS[currentTier];

        // Lógica simplificada de prorrateo (simulada)
        const now = new Date();
        const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        const remainingDays = daysInMonth - now.getDate();
        const prorationFactor = remainingDays / daysInMonth;

        const credit = currentPlan.baseFee * prorationFactor;
        const newCost = newPlan.baseFee * prorationFactor;
        const dueNow = Math.max(0, newCost - credit);
    ...
    /**
     * Actualiza manualmente la suscripción de un tenant (Admin Override)
     */
    static async manualUpdateSubscription(tenantId: string, updates: any, metadata?: { performedBy: string, correlationId?: string }) {
        const config = await TenantService.getConfig(tenantId);

        // Merge updates into subscription
        ...
        // Use TenantService to persist and audit
        await TenantService.updateConfig(tenantId, { subscription: newSubscription }, metadata);
    }
}
