...
export class SpaceInvitationService {
    ...
    static async validateToken(token: string): Promise<SpaceInvitation> {
        ...
        if (!invitation) {
            throw new AppError('NOT_FOUND', 404, 'La invitación no es válida o ha expirado');
        }
    ...
    static async acceptInvitation(token: string, userId: string, session?: ClientSession): Promise<void> {
        ...
        if (!invitation) {
            throw new AppError('NOT_FOUND', 404, 'Invitación no válida');
        }
    ...
    static async revokeInvitation(token: string, session?: ClientSession): Promise<void> {
        ...
        if (result.matchedCount === 0) {
            throw new AppError('NOT_FOUND', 404, 'Invitación no encontrada o ya procesada');
        }
    }
}
