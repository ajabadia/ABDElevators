...
import { Palette, Upload, Loader2, Building, Trash2, FileText } from "lucide-react";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
                <div className="space-y-4">
                    ...
                    <p className="text-slate-500">Personaliza la apariencia de los informes técnicos generados.</p>
                </div>

                <div className="space-y-6">
                    <div className="space-y-4">
                        <Label className="text-slate-700 font-semibold">Legal Disclaimer</Label>
                        <textarea
                            className="flex min-h-[80px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                            placeholder="Texto legal al pie de cada informe..."
                            value={config?.branding?.reportSettings?.legalDisclaimer || ''}
                            onChange={(e) => setConfig(prev => prev ? {
                                ...prev,
                                branding: {
                                    ...(prev.branding || {}),
                                    reportSettings: {
                                        ...(prev.branding?.reportSettings || {}),
                                        legalDisclaimer: e.target.value
                                    }
                                }
                            } : null)}
                        />
                        <p className="text-[11px] text-slate-400">Este texto aparecerá en letras pequeñas al final de cada página.</p>
                    </div>

                    <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-3">
                            <Label className="text-slate-700 font-semibold">Footer Text</Label>
                            <Input
                                placeholder="Confidencial - Propiedad de..."
                                value={config?.branding?.reportSettings?.footerText || ''}
                                onChange={(e) => setConfig(prev => prev ? {
                                    ...prev,
                                    branding: {
                                        ...(prev.branding || {}),
                        ...
                        <div className="space-y-3">
                            <Label className="text-slate-700 font-semibold">Color Impresión</Label>
                            <div className="flex gap-3">
                                <div
                                    ...
                                    style={{ backgroundColor: config?.branding?.reportSettings?.primaryColor || config?.branding?.colors?.primary || '#000000' }}
                                >
                                    <input
                                        type="color"
                                        className="opacity-0 w-full h-full cursor-pointer"
                                        value={config?.branding?.reportSettings?.primaryColor || config?.branding?.colors?.primary || '#000000'}
                                        onChange={(e) => setConfig(prev => prev ? {
                                            ...prev,
                                            branding: {
                                                ...(prev.branding || {}),
                                ...
                                <Input
                                    value={config?.branding?.reportSettings?.primaryColor || ''}
                                    placeholder={config?.branding?.colors?.primary}
                                    onChange={(e) => setConfig(prev => prev ? {
                                        ...prev,
                                        branding: {
