...
export class BillingAdminService {
    ...
    static async getTenantContracts(page: number = 1, limit: number = 10, search?: string): Promise<{ contracts: ContractSummary[], total: number }> {
        ...
        const contracts = await Promise.all(tenants.map(async (tenant: any) => {
            try {
                ...
                if (!sub) {
                    console.warn(`Tenant ${config.tenantId} has no subscription data. Skipping.`);
                    return null; // Handle filter later
                }
                const plan = PLANS[sub.planSlug as keyof typeof PLANS] || PLANS.FREE;
                ...
                const tokensStart = sub.overrides?.['llm_tokens_per_month'] || plan.limits['llm_tokens_per_month'] || plan.limits['llm_tokens_per_month'] || 0;
                const storageStart = sub.overrides?.['storage_bytes'] || plan.limits['storage_bytes'] || plan.limits['storage_bytes'] || 0;

                const tokensCost = BillingEngine.calculateMetricCost(usageStats.tokens, tokensStart);
                const storageCost = BillingEngine.calculateMetricCost(usageStats.storage, storageStart);
        ...
        })).then(results => results.filter((c): c is ContractSummary => c !== null));
    ...
    static async updateContract(tenantId: string, data: {
        ...
        // 1. Tier Change
        if (data.tier && sub && data.tier !== sub.planSlug) {
