...
export function useApiMutation<T = any, R = any>({
    ...
    const executeMutation = useCallback(async (variables: T, shouldThrow: boolean = false) => {
    ...
    const mutate = useCallback((variables: T) => executeMutation(variables, false), [executeMutation]);
    const mutateAsync = useCallback((variables: T) => executeMutation(variables, true), [executeMutation]);
