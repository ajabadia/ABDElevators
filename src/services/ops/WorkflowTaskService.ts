...
import type { WorkflowTask } from '@/lib/schemas';
...
export class WorkflowTaskService {
    ...
    static async getTaskStats(tenantId: string) {
        const collection = await getTenantCollection(this.COLLECTION);
        const tasks = await collection.find({ tenantId });

        const pending = tasks.filter((t: any) => t.status === 'PENDING').length;
        const inReview = tasks.filter((t: any) => t.status === 'IN_PROGRESS' || t.status === 'UNDER_REVIEW').length;
        ...
        const completedTasks = tasks.filter((t: any) => t.status === 'COMPLETED' && t.completedAt && t.createdAt);
        const totalDuration = completedTasks.reduce((acc: number, t: any) => {
