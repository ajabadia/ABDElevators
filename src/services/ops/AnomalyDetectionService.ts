        ...
        try {
            ...
            if (metric === 'durationMs') {
                ...
                return {
                    avg: result.avg || 0,
                    stdDev: result.stdDev || 0,
                    count: 0 // Count not used for error baseline in the same way
                };
            }
        } catch (error: unknown) {
            console.error('[AnomalyDetectionService] Error calculating baseline:', error);
        ...
        try {
            ...
            for (const source of sources) {
                ...
                if (zScore > 2.0) {
                    ...
                    if (severity === 'CRITICAL' || severity === 'HIGH') await this.reportAnomaly(anomaly);
                }
            }
            return anomalies;
        } catch (error: unknown) {
            console.error('[AnomalyDetectionService] Error detecting latency anomalies:', error);
            throw new AppError('INTELLIGENCE_ERROR', 500, 'Error en detección de anomalías de latencia');
        }
    }
        ...
        try {
            ...
            for (const source of sources) {
                ...
                if (baseline.avg === 0 && recentCount > 5) {
                    const anomaly = this.createErrorAnomaly(source, recentCount, 0, 3.0, 'HIGH');
                    anomalies.push(anomaly);
            ...
            console.error('[AnomalyDetectionService] Error detecting error anomalies:', error);
            throw new AppError('INTELLIGENCE_ERROR', 500, 'Error en detección de anomalías de errores');
        }
    }

    private static createErrorAnomaly(source: string, current: number, baseline: number, zScore: number, severity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL'): Anomaly {
        return {
