...
import { renderLimitAlertEmail } from '@/services/infra/emails/limit-alert';
import { renderPaymentFailedEmail } from '@/services/infra/emails/payment-failed';
import { renderInvitationEmail } from '@/services/infra/emails/invitation';
import { renderMfaEnabledEmail } from '@/services/infra/emails/mfa-enabled';
import { renderNewInvoiceEmail } from '@/services/infra/emails/new-invoice';



export class EmailService {
    static async sendLimitAlert(params: {
        to: string;
        tenantName: string;
    ...
    static async sendMfaEnabledEmail(params: {
        to: string;
        userName: string;
    }): Promise<void> {
    ...
    static async sendNewInvoiceNotification(params: {
        to: string;
        tenantName: string;
    ...
    static async sendReportDelivery(params: {
        to: string;
        reportName: string;
        templateType: string;
        generatedAt: Date;
        ...
        const { renderReportDeliveryEmail } = await import('@/services/infra/emails/report-delivery');
        const resend = getResend();
