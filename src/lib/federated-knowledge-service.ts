...
export class FederatedKnowledgeService {
     ...
     */
    static async searchGlobalPatterns(query: string, limit: number = 3): Promise<FederatedPattern[]> {
        const db = await connectDB();
        ...
        // Fallback: Keyword regex match (Simple V1)
        // We will prioritize an actual vector match logic later in Phase 17

        const patterns = await db.collection('federated_patterns')
            .find({ status: 'PUBLISHED' })
            .limit(limit * 2) // Fetch more to filter locally if needed
            .toArray();
