 ...
 * Implementa múltiples capas de recuperación y validación con Zod.
 * Fase 165.2: LLM JSON Parsing Resilience
 */
export async function safeParseLlmJson<T>(params: {
    ...
    try {
        ...
        // Capa 2: Extracción con Regex de bloque {...}
        method = 'REGEX_MATCH';
        ...
        if (jsonMatch) {
            try {
                ...
                // Capa 3: Limpieza de Markdown fences si existen
                ...
                try {
                    ...
                    // Capa 4: Intento de reparación básica (comas finales, etc.)
                    // Nota: Aquí se podrían usar librerías como json5 si se desea más robustez
                    method = 'REPAIR_ATTEMPT';
                    const repaired = jsonMatch[0]
                        .replace(/,\s*([\]}])/g, '$1') // Remueve comas antes de ] o }
                        .trim();
