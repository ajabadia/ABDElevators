            ...
            <section className="pt-32 pb-20 px-6 relative overflow-hidden">
                ...
                <div className="container mx-auto max-w-6xl relative z-10">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center border border-emerald-500/20 shadow-sm">
            ...
            <section className="pb-20 px-6">
                <div className="container mx-auto max-w-6xl">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-24">
                        <div className="p-10 bg-red-500/5 border border-red-500/10 rounded-xl">
                        ...
                        <div className="p-10 bg-emerald-500/5 border border-emerald-500/10 rounded-xl relative overflow-hidden">
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-8 flex items-center gap-3 font-outfit tracking-tight">
                    ...
                    {/* How it Works */}
                    <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl p-10 md:p-16 mb-24">
                    ...
                    {/* Example Output */}
                    <div className="mb-24">
                        <h2 className="text-4xl font-bold text-slate-900 dark:text-white mb-12 font-outfit tracking-tight text-center">{t("example_title")}</h2>
                        <div className="p-1 md:p-1.5 bg-slate-200 dark:bg-slate-800 rounded-xl shadow-lg">
                            <div className="p-8 md:p-12 bg-white dark:bg-slate-950 rounded-xl space-y-12">
                                <div className="space-y-4">
                                    <p className="text-emerald-600 dark:text-emerald-400 font-bold text-xs font-mono">{t("example_query_label")}</p>
                                    <p className="text-2xl md:text-3xl font-medium text-slate-900 dark:text-white tracking-tight italic">
                                        "{t("example_query")}"
                                    </p>
                                </div>

                                <div className="space-y-6">
                                    <p className="text-emerald-600 dark:text-emerald-400 font-bold text-xs font-mono">{t("example_resp_label")}</p>
                                    <div className="p-8 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-white/5 space-y-6">
                                        <p className="text-slate-700 dark:text-slate-200 text-xl leading-relaxed">
                                            {t("example_resp_text")}<sup className="text-emerald-600 dark:text-emerald-400 font-bold ml-1">[1]</sup>
                                        </p>
                                ...
                                <div className="space-y-6">
                                    <p className="text-emerald-600 dark:text-emerald-400 font-bold text-xs font-mono">{t("example_ref_label")}</p>
                    ...
                    <div className="p-16 bg-slate-900 text-center rounded-xl relative overflow-hidden group">
                        ...
                        <Link href="/login">
                            <Button className="bg-emerald-600 hover:bg-emerald-500 text-white font-black text-2xl px-16 py-8 rounded-xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-emerald-900/20">
                                {t("cta_btn")}
                            </Button>
                        </Link>
                    </div>
...
function AuditStep({ number, title, description, icon }: { number: string; title: string; description: string; icon: React.ReactNode }) {
    return (
        <div className="flex gap-6 group p-8 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-xl transition-all duration-300">
            <div className="flex-shrink-0 w-16 h-16 bg-emerald-500/10 rounded-xl flex items-center justify-center text-emerald-600 dark:text-emerald-400 text-2xl font-black border border-emerald-500/20 group-hover:bg-emerald-500/20 transition-all">
                {number}
            </div>
    ...
    return (
        <div className="flex flex-col gap-4 p-6 bg-slate-900 border border-white/5 rounded-2xl hover:border-emerald-500/30 transition-all cursor-pointer group hover:-translate-y-1">
            <div className="flex items-center justify-between font-mono">
                <div className="w-8 h-8 bg-emerald-500/10 rounded-lg flex items-center justify-center text-emerald-600 dark:text-emerald-400 text-sm font-bold border border-emerald-500/20">
                    {number}
                </div>
                <span className="text-slate-400 dark:text-slate-500 text-[10px] uppercase">Page {page}</span>
            </div>
...
function ComplianceBadge({ name, description }: { name: string; description: string }) {
    return (
        <div className="p-8 bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-white/5 rounded-xl text-center group">
            <div className="w-16 h-16 bg-emerald-500/10 rounded-full flex items-center justify-center mx-auto mb-6 border border-emerald-500/10">
                <ShieldCheck className="text-emerald-600 dark:text-emerald-400" size={32} />
            </div>
            <p className="text-slate-900 dark:text-white font-black text-xl mb-2 font-outfit tracking-tight">{name}</p>
